...
import { fetchMarketFailure, fetchMarketRequest, fetchMarketSuccess, Dispatch } from './actions';
...
export const fetchMarket = (ids: string[]) => async (dispatch: Dispatch): Promise<void> => {
  ...
  const res = await fetch(
    `${apiBaseUrl}/simple/price?ids=${idsString}&vs_currencies=${currenciesString}`,
  );
